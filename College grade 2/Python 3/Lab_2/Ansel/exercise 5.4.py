...
def main():
    maxwidth, format = process_options() #Call the value of the process_options
...
def process_options():
    ...
    maxwidth = 100
    format = ".0f"
    for arg in sys.argv[1:]: #If you type usage on the command line
        if arg in ["-h", "--help"]:
            print("""\
            usage:
            ...
            return None, None
        elif arg.startswith(maxwidth_arg): #If the command line contains fields that begin with "maxwidth="
            try:
                maxwidth = int(arg[len(maxwidth_arg):])
            except ValueError:
                pass
        elif arg.startswith(format_arg):#If the command line contains fields that begin with "format="
